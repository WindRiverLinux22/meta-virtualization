...
do_install () {
    ...
    if [ -e ${D}/usr/bin/crit ]; then
	sed -i 's%^\#\!.*%\#\!/usr/bin/env python3%' ${D}/usr/bin/crit
    fi
    if [ -e ${D}${libdir}/python3*/site-packages/crit-0.0.1-py3*.egg/EGG-INFO/scripts/crit ]; then
	sed -i 's%^\#\!.*%\#\!/usr/bin/env python3%' ${D}/${libdir}/python3*/site-packages/crit-0.0.1-py3*.egg/EGG-INFO/scripts/crit
    fi
}
